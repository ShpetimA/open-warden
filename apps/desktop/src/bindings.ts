...
async getCommitHistory(repoPath: string, limit: number | null) : Promise<Result<HistoryCommit[], ApiError>> {
    try {
...
async getCommitFileVersions(repoPath: string, commitId: string, relPath: string, previousPath: string | null) : Promise<Result<FileVersions, ApiError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_commit_file_versions", { repoPath, commitId, relPath, previousPath }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async discardFile(repoPath: string, relPath: string, bucket: Bucket) : Promise<Result<null, ApiError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("discard_file", { repoPath, relPath, bucket }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type FileStatus = "added" | "deleted" | "renamed" | "copied" | "type-changed" | "unmerged" | "modified" | "untracked"
export type FileVersions = { oldFile: DiffFile | null; newFile: DiffFile | null }
export type GitSnapshot = { repoRoot: string; branch: string; unstaged: FileItem[]; staged: FileItem[]; untracked: FileItem[] }
export type HistoryCommit = { commitId: string; shortId: string; summary: string; author: string; relativeTime: string }
