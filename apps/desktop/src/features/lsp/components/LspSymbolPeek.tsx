...
export function LspSymbolPeek({ document, containerRef }: SymbolPeekProps) {
  ...
  return (
    <div
      className="absolute right-0 left-0 z-20 border border-white/12 bg-[#1e1e1e] text-[#cccccc] shadow-[0_0_0_1px_rgba(0,0,0,0.55)]"
      style={{
        top: `${popoverTop}px`,
        height: `${SYMBOL_PEEK_HEIGHT_REM}rem`,
      }}
    >
      <div className="flex h-7 items-center gap-2 border-b border-white/10 px-2 text-[11px]">
        <div className="truncate text-[#9cdcfe]">
          {activeLocation?.relPath ?? document.relPath}
        </div>
        <div className="truncate text-[#858585]">- {getSymbolPeekTitle(symbolPeek.kind, locations.length)}</div>
        <button
          type="button"
          className="ml-auto inline-flex h-5 w-5 items-center justify-center text-[#858585] hover:bg-white/6 hover:text-[#cccccc]"
          onClick={closePeek}
      ...
      <div className="grid h-[calc(100%-1.75rem)] min-h-0 grid-cols-[minmax(0,1fr)_20rem]">
        <section className="min-h-0 min-w-0 border-r border-white/10 bg-[#1e1e1e]">
          {previewError ? (
            <div className="px-3 py-2 text-xs text-[#f48771]">{previewError}</div>
          ) : previewQuery.isFetching && previewFile === undefined ? (
            <div className="px-3 py-2 text-xs text-[#858585]">Loading preview...</div>
          ) : (
            <SymbolPeekPreview location={activeLocation} contents={previewFile?.contents ?? null} />
          )}
        </section>

        <section className="flex min-h-0 min-w-0 flex-col bg-[#252526]">
          <div className="flex h-7 items-center gap-2 border-b border-white/10 px-2">
            ...
            <input
              ...
              className="h-full min-w-0 flex-1 bg-transparent text-[11px] text-[#cccccc] outline-none placeholder:text-[#6a6a6a]"
              placeholder="Filter"
            />
          </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {groups.length === 0 ? (
              <div className="px-2 py-2 text-[11px] text-[#858585]">No matching symbols.</div>
            ) : (
              groups.map((group) => (
                <div key={group.relPath} className="border-b border-white/5 last:border-b-0">
                  <div className="flex h-6 items-center gap-2 px-2 text-[11px] text-[#cccccc]">
                    <span className="min-w-0 flex-1 truncate">{group.relPath}</span>
                    <span className="text-[#4fc1ff]">{group.items.length}</span>
                  </div>
                  {group.items.map(({ index, location }) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className={`block w-full border-t border-white/5 px-2 py-1.5 text-left text-[11px] ${
                          isActive ? "bg-[#37373d]" : "hover:bg-white/6"
                        }`}
                        onClick={() => {
                      ...
                      >
                        <div className="text-[#cccccc]">{lineLabel(location)}</div>
                        <div className="truncate pt-0.5 text-[#9cdcfe]">
